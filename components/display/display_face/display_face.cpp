...
static void circ(int cx, int cy, int r, bool on = true) { for (int y = -r; y <= r; ++y) { int q = r * r - y * y; int dx = q > 0 ? (int)sqrtf((float)q) : 0; for (int x = -dx; x <= dx; ++x) px(cx + x, cy + y, on); } }
...
void display_face_render_mochi_gaze(int expr,int step,int sX,int sY,int gx,int gy,int ex,int ey){memset(s_fb,0,sizeof(s_fb));ears(sY);int l=34+sX+ex,r=94+sX+ex,y=28+sY+ey; if(expr==6){sadbrow(l,14+sY,true);sadbrow(r,14+sY,false);sadeye(l,y,gy);sadeye(r,y,gy);} else if(expr==2&&step==2){happy_eye(l,y);happy_eye(r,y);brow(l,14+sY,1);brow(r,14+sY,1);mouth(2,sY);} else if(expr==99){erroreye(l,y,0);erroreye(r,y,0);brow(l,14+sY,3);brow(r,14+sY,3);mouth(2,sY);} else if(step==3){sleep_eye(l,y);sleep_eye(r,y);mouth(0,sY);} else if(step==1){blink_eye(l,y,0);blink_eye(r,y,0);brow(l,14+sY);brow(r,14+sY);} else {eye_render:; circ(l,y,expr==1?14:13);circ(r,y,expr==1?14:13);circ(l+clampi(gx,-7,7),y+clampi(gy,-5,5),6,false);circ(r+clampi(gx,-7,7),y+clampi(gy,-5,5),6,false);brow(l,14+sY);brow(r,14+sY);if(expr==1){attentive(l,14+sY);attentive(r,14+sY);mouth(1,sY);}else mouth(0,sY);} }
