...
static void reset_state() { s_buffer_samples = 0; }
...
extern "C" bool wakeword_init(void)
{
    ...
    if (!s_iface) { esp_srmodel_deinit(s_models); s_models = nullptr; return false; }
    s_model = s_iface->create(MODEL_NAME, DET_MODE_90);
    if (!s_model) { s_iface = nullptr; esp_srmodel_deinit(s_models); s_models = nullptr; return false; }
    ...
    reset_state();
    ESP_LOGI(TAG, "WakeNet ready: model=%s rate=%dHz chunk=%d channels=%d", MODEL_NAME, s_sample_rate, s_chunk_samples, s_channels);
    if (s_sample_rate != 16000 || s_channels != 1 || s_chunk_samples <= 0 || s_chunk_samples > (int)BUFFER_SAMPLES) {
        wakeword_deinit(); return false;
    }
...
{
    if (!samples || sample_count == 0 || !s_iface || !s_model || s_chunk_samples <= 0) return false;
    while (sample_count > 0) {
        ...
        s_buffer_samples += copy_samples; samples += copy_samples; sample_count -= copy_samples;
        while (s_buffer_samples >= (size_t)s_chunk_samples) {
            const int result = s_iface->detect(s_model, s_buffer);
            if (result > 0) {
                ESP_LOGW(TAG, "WAKE WORD TERDETEKSI: HI, ESP (id=%d)", result);
                reset_state(); return true;
            }
            const size_t remaining = s_buffer_samples - (size_t)s_chunk_samples;
            if (remaining > 0) memcpy(s_buffer, s_buffer + s_chunk_samples, remaining * sizeof(int16_t));
            s_buffer_samples = remaining;
        }
    }
    return false;
}
extern "C" int wakeword_get_chunk_samples(void) { return s_chunk_samples; }
extern "C" int wakeword_get_sample_rate(void) { return s_sample_rate; }
extern "C" void wakeword_deinit(void)
{
    reset_state();
    if (s_model && s_iface) s_iface->destroy(s_model);
    s_model = nullptr; s_iface = nullptr; s_chunk_samples = 0; s_sample_rate = 0; s_channels = 0;
    if (s_models) { esp_srmodel_deinit(s_models); s_models = nullptr; }
}
