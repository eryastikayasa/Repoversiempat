...
void websocket_event_handler(void*args,esp_event_base_t base,int32_t event_id,void*event_data){(void)args;(void)base;esp_websocket_event_data_t *d=(esp_websocket_event_data_t*)event_data;websocket_transport_handle_event(event_id,event_data);websocket_tx_handle_event(event_id);switch(event_id){case WEBSOCKET_EVENT_CONNECTED:s_gemini_ready=false;reset_rx();ensure_rx_worker();break;case WEBSOCKET_EVENT_DATA:if(d&&d->op_code==0x08){ESP_LOGW(TAG,"Gemini CLOSE frame received");if(d->data_ptr&&d->data_len>=2){uint16_t code=((uint8_t)d->data_ptr[0]<<8)|(uint8_t)d->data_ptr[1];ESP_LOGW(TAG,"Gemini CLOSE code=%u (0x%04X), payload_len=%d",(unsigned)code,(unsigned)code,(int)d->data_len);if(d->data_len>2){size_t n=(size_t)d->data_len-2;if(n>120)n=120;char reason[121];memcpy(reason,d->data_ptr+2,n);reason[n]='\0';ESP_LOGW(TAG,"Gemini CLOSE reason: %s",reason);}}break;}handle_data_event(d);break;case WEBSOCKET_EVENT_ERROR:if(d){ESP_LOGE(TAG,"WS ERROR detail: type=%d sock_errno=%d tls_esp_err=0x%x tls_stack_err=0x%x handshake=%d",(int)d->error_handle.error_type,d->error_handle.esp_transport_sock_errno,(unsigned)d->error_handle.esp_tls_last_esp_err,(unsigned)d->error_handle.esp_tls_stack_err,d->error_handle.esp_ws_handshake_status_code);}s_gemini_ready=false;reset_rx();break;case WEBSOCKET_EVENT_DISCONNECTED:s_gemini_ready=false;reset_rx();break;default:break;}}
