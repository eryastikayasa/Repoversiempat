...
static uint32_t s_last_legacy_generation = 0;
...
static void sync_task(void *arg)
{
    (void)arg;
    ESP_LOGI(TAG,
             "AudioEngine control plane: output=%uHz PCM16, prebuffer=%uB (~%ums), warning=%uB (~%ums), critical=%uB (~%ums)",
...
bool audio_engine_init(void)
{
    ...
    s_initialized = true;
    s_last_legacy_generation = websocket_connection_generation;
    reset_turn(s_last_legacy_generation);
    ...
    ESP_LOGI(TAG, "AudioEngine aktif: 1 otak audio / legacy playback bridge");
    return true;
}
...
void audio_engine_notify(audio_engine_event_type_t event, uint32_t generation)
{
    ...
    if (generation != 0 && generation != s_turn.generation) {
        ...
        s_last_legacy_generation = generation;
    ...
    switch (event) {
        ...
        case AUDIO_ENGINE_EVENT_MODEL_AUDIO:
            s_turn.model_started = true;
            if (s_state == AUDIO_ENGINE_THINKING || s_state == AUDIO_ENGINE_IDLE || s_state == AUDIO_ENGINE_LISTENING)
                set_state(AUDIO_ENGINE_BUFFERING);
            break;
...
void audio_engine_sync_legacy_state(void)
{
    ...
    if (generation != s_last_legacy_generation) {
        s_last_legacy_generation = generation;
        reset_turn(generation);
        set_state(AUDIO_ENGINE_IDLE);
        return;
    ...
    if (s_turn.model_complete) {
        if (s_turn.pending_bytes > 0) {
            if (s_state != AUDIO_ENGINE_DRAINING)
                set_state(AUDIO_ENGINE_DRAINING);
        } else if (s_state != AUDIO_ENGINE_COMPLETE && s_state != AUDIO_ENGINE_IDLE) {
            /* Legacy completion already performs the final I2S drain. */
            set_state(AUDIO_ENGINE_COMPLETE);
            set_state(AUDIO_ENGINE_IDLE);
        }
        return;
    ...
    if (audio_turn_active) {
        ...
        if (s_state == AUDIO_ENGINE_PLAYING && s_turn.pending_bytes < ENGINE_CRITICAL_BYTES)
            ...
            set_state(AUDIO_ENGINE_PLAYING);
    } else if (s_state == AUDIO_ENGINE_COMPLETE) {
        set_state(AUDIO_ENGINE_IDLE);
    }
}

/* Transitional bootstrap: keep the current playback path intact while the
 * AudioEngine becomes the single control-plane authority. */
__attribute__((constructor)) static void audio_engine_bootstrap(void)
{
    /* FreeRTOS objects can be created during ESP-IDF C++ static init, but the
     * control task itself only starts running once the scheduler is active. */
