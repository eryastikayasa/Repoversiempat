...
static bool compact_large_audio_payload(char *json, size_t *io_len)
{
    ...
    if (!audio_engine_push_model_audio_base64(b64, b64_len, websocket_connection_generation))
        ESP_LOGW(TAG, "AudioEngine gagal ingest Base64 audio besar: len=%u", (unsigned)b64_len);
...
static void add_device_control_tool(cJSON *setup)
{
    ...
    cJSON_AddStringToObject(decl, "description", "Mengontrol perangkat rumah melalui UART. Gunakan hanya jika pengguna meminta aksi perangkat. Setelah aksi berhasil, jawab pengguna secara natural dalam bahasa Indonesia dan jangan menyebut nama command UART.");
    ...
    static const char *const commands[] = {"r1","r2","r3","r4","fan_pwr","fan_speed","fan_swing","fan_mode","mp3_mode","mp3_play","mp3_eq","m_led","m_mute","m_musik","m_cek","cek_suhu","cek_cahaya","face_idle","face_listening","face_thinking","face_speaking","face_happy","face_sad","face_error","face_sleep"};
...
bool build_gemini_setup(char **output, size_t *output_len)
{
    ...
    *output_len = 0;
    ...
    if (web_config_load_role(role_text, sizeof(role_text)) && role_text[0] != '\0') {
        ...
        if (role_part) { cJSON_AddStringToObject(role_part, "text", role_text); cJSON_AddItemToArray(system_parts, role_part); }
    }
    ...
    if (session_resumable && session_handle[0] != '\0') cJSON_AddStringToObject(resumption, "handle", session_handle);
    char *json = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    if (!json) { ESP_LOGE(TAG, "Gagal serialize Gemini setup JSON"); return false; }
    *output = json;
...
static cJSON *parse_json_with_diagnostics(const char *json, size_t len)
{
    ...
    if (error_ptr && error_ptr >= json && error_ptr < json + len) error_offset = (size_t)(error_ptr - json);
    else if (parse_end && parse_end >= json && parse_end <= json + len) error_offset = (size_t)(parse_end - json);
    ...
    cJSON_ArrayForEach(fc, function_calls) {
        ...
        if (strcmp(name->valuestring, "control_device") == 0) {
            ...
            if (cJSON_IsString(command) && command->valuestring) {
                ...
                if (strcmp(cmd, "face_idle") == 0) { display_face_show_for_ms(FACE_IDLE, 5000); success = true; }
                else if (strcmp(cmd, "face_listening") == 0) { display_face_show_for_ms(FACE_LISTENING, 5000); success = true; }
                else if (strcmp(cmd, "face_thinking") == 0) { display_face_show_for_ms(FACE_THINKING, 5000); success = true; }
                else if (strcmp(cmd, "face_speaking") == 0) { display_face_show_for_ms(FACE_SPEAKING, 5000); success = true; }
                else if (strcmp(cmd, "face_happy") == 0) { display_face_show_for_ms(FACE_HAPPY, 5000); success = true; }
                else if (strcmp(cmd, "face_sad") == 0) { display_face_show_for_ms(FACE_SAD, 5000); success = true; }
                else if (strcmp(cmd, "face_error") == 0) { display_face_show_for_ms(FACE_ERROR, 5000); success = true; }
                else if (strcmp(cmd, "face_sleep") == 0) { display_face_show_for_ms(FACE_SLEEP, 5000); success = true; }
                else { success = uart_control_execute_command(cmd); ESP_LOGI(TAG, "UART TOOL command=%s result=%s", cmd, success ? "OK" : "REJECTED"); }
            }
        }
        if (!websocket_send_tool_response(id->valuestring, name->valuestring, success)) ESP_LOGW(TAG, "Gagal mengirim toolResponse ke Gemini");
    }
}
...
void process_gemini_message(const char *json, size_t len)
{
    ...
    if (cJSON_IsObject(input_transcription)) {
        ...
        if (cJSON_IsString(text) && text->valuestring) { ESP_LOGI(TAG, "USER: %s", text->valuestring); display_text_set_user(text->valuestring); }
    }
    ...
    if (cJSON_IsObject(output_transcription)) {
        ...
        if (cJSON_IsString(text) && text->valuestring) { ESP_LOGI(TAG, "GEMINI TEXT: %s", text->valuestring); display_text_set_gemini(text->valuestring); }
    }
    cJSON *server = cJSON_GetObjectItem(root, "serverContent");
    if (cJSON_IsObject(server)) {
        cJSON *interim = cJSON_GetObjectItem(server, "interimInputTranscription");
        if (cJSON_IsObject(interim)) { cJSON *text = cJSON_GetObjectItem(interim, "text"); if (cJSON_IsString(text) && text->valuestring) display_text_set_user(text->valuestring); }
        cJSON *server_in = cJSON_GetObjectItem(server, "inputTranscription");
        if (cJSON_IsObject(server_in)) { cJSON *text = cJSON_GetObjectItem(server_in, "text"); if (cJSON_IsString(text) && text->valuestring) { ESP_LOGI(TAG, "USER: %s", text->valuestring); display_text_set_user(text->valuestring); } }
        cJSON *server_out = cJSON_GetObjectItem(server, "outputTranscription");
        if (cJSON_IsObject(server_out)) { cJSON *text = cJSON_GetObjectItem(server_out, "text"); if (cJSON_IsString(text) && text->valuestring) { ESP_LOGI(TAG, "GEMINI TEXT: %s", text->valuestring); display_text_append_gemini(text->valuestring); } }
        ...
        if (cJSON_IsObject(turn)) {
            ...
            if (cJSON_IsArray(parts)) {
                ...
                cJSON_ArrayForEach(part, parts) {
                    ...
                    if (cJSON_IsString(mimeType) && mimeType->valuestring) ESP_LOGD(TAG, "Gemini audio MIME: %s", mimeType->valuestring);
                    ...
                    if (b64_len > 0) (void)audio_engine_push_model_audio_base64(audio->valuestring, b64_len, websocket_connection_generation);
                }
            }
        }
    }
